...
fn day1(input: &InputInfo) {
    ...
    input.lines_info.iter().for_each(|line_info: &LineInfo| {
        ...
        line_info.handfuls.iter().for_each(|handful: &Handful| {
            if handful.red_nb > max_red
                || handful.green_nb > max_green
                || handful.blue_nb > max_blue
            {
                possible = false;
            }
        });
...
impl InputInfo {
    pub fn parse_input(input: &str) -> Self {
        ...
        input
            .lines()
            .for_each(|line| lines_info.push(LineInfo::parse_line(line)));
        InputInfo { lines_info }
    }
}
impl LineInfo {
    pub fn parse_line(line: &str) -> Self {
        ...
        handful_list
            .split("; ")
            .for_each(|handful| handful_vec.push(Handful::parse_handful(handful)));
